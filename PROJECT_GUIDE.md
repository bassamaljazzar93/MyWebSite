...
```
MyWebSite/
├── index.html              # الصفحة الرئيسية - بورتفوليو بسام الجزار (قالب جديد)
├── ai-customizer.html      # محرر البورتفوليو بالذكاء الصناعي
...
### 1. الصفحة الرئيسية (index.html) - قالب جديد

تم استبدال الصفحة الرئيسية بقالب جديد بتصميم مستوحى من واجهة سطر الأوامر (CLI) مع عارض ثلاثي الأبعاد تفاعلي. تتيح هذه الواجهة للمستخدمين التفاعل مع البورتفوليو عبر أوامر نصية.

#### التحديثات الرئيسية:
- **تصميم CLI**: واجهة مستخدم جديدة تعتمد على سطر الأوامر.
- **عارض ثلاثي الأبعاد**: دمج Spline 3D viewer لعرض نماذج ثلاثية الأبعاد تفاعلية.
- **قائمة التنقل**: تم تحديث قائمة التنقل لتشمل روابط إلى:
    - `✨ AI Portfolio` (تؤدي إلى `auth.html` لبدء عملية التخصيص).
    - `🤖 chatbot` (ملعب الذكاء الصناعي).
- **أوامر جديدة**: إضافة أمر `portfolio` في واجهة CLI والذي يقوم بتحويل المستخدم إلى صفحة `auth.html`.

### 2. صفحة AI Customizer (ai-customizer.html)
...
1. **الصفحة الرئيسية** (index.html)
   - تفاعل مع واجهة CLI أو استخدم قائمة التنقل.
   - للبدء في بناء بورتفوليو بالذكاء الصناعي، استخدم أمر `portfolio` أو انقر على رابط `✨ AI Portfolio`.
...
2. **التسجيل/تسجيل الدخول** (auth.html)
   ...
   - بعد تسجيل الدخول بنجاح، سيتم تحويلك تلقائياً إلى صفحة التخصيص.
